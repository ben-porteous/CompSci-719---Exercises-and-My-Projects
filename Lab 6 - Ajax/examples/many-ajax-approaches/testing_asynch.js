...
/*    Chaining fetch requests with async await*/
    async function chainRequestsAsyncAwait() {
        let articleResponse = await fetch(`https://article.trex-sandwich.com/articles?id=1`);
        let articleJson = await articleResponse.json();

        let userResponse = await fetch(`https://article.trex-sandwich.com/users?id=${articleJson.author_id}`);
    ...
    async function getFullArticleObj(articleId) {
        let articleResponseObj = await fetch(`https://article.trex-sandwich.com/articles?id=${articleId}`);
        let articleJson = await articleResponseObj.json();
        return articleJson;
    }

    async function getUserObj(userId) {
        let userResponseObj = await fetch(`https://article.trex-sandwich.com/users?id=${userId}`);
    ...
    function chainRequestsDotThen() {
        let articleObject;
        fetch(`https://article.trex-sandwich.com/articles?id=1`)
            ...
            .then(function (articleJson) {
                ...
                return fetch(`https://article.trex-sandwich.com/users?id=${articleJson.author_id}`)
            })
    ...
    function chainVanillaXhrWithNesting() {
        ...
        xhrArticle.onreadystatechange = function () {
            if (xhrArticle.readyState === XMLHttpRequest.DONE && xhrArticle.status === 200) {
                ...
                xhrAuthor.open("GET", `https://article.trex-sandwich.com/users?id=${articleJson.author_id}`, true);
                xhrAuthor.send();

            }
        };

        xhrArticle.open("GET", `https://article.trex-sandwich.com/articles?id=1`, true);
        xhrArticle.send();
    ...
    function runPromiseReturningXHRGeneric() {
        ...
        promiseReturningXHRGeneric(`https://article.trex-sandwich.com/articles?id=1`)
            .then(function (articleJson) {
                articleObject = articleJson;
                return promiseReturningXHRGeneric(`https://article.trex-sandwich.com/users?id=${articleJson.author_id}`);
            })
            .then(function (userJson) {
